...
test.describe("Home page (/home)", () => {
  ...
  test("desktop nav links are visible after the header is revealed", async ({ page, viewport }) => {
    ...
    await expect(nav.getByRole("link", { name: "Recomendaciones", exact: true })).toBeVisible();
  ...
  test("hero subtitle is visible", async ({ page }) => {
    await expect(page.getByText(/Sé parte de una de las redes de ayuda/i)).toBeVisible({
      timeout: 5000,
    });
  });
  ...
  test("footer renders with org name and contact info", async ({ page }) => {
    ...
    await expect(
      footer.getByText("(787)-505-8255", {
        exact: true,
      })
    ).toBeVisible();
    await expect(
      footer.getByText("info@animalrescue.org", {
        exact: true,
      })
    ).toBeVisible();
  });
