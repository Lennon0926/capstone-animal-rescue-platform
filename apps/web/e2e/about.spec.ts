...
test.describe('About page (/about)', () => {
  ...
  test('hero tagline is visible', async ({page}) => {
    await expect(page.getByText('Desde 1990 Protegiendo Vidas', {
      exact: true
    })).toBeVisible();
  });

  test('hero org name heading is visible', async ({page}) => {
    // Scoped to the hero section (first section) to avoid matching the footer
    // brand text
    await expect(page.locator('section').first().getByRole('heading', {
  ...
  test('hero subtitle about volunteers is visible', async ({page}) => {
    await expect(page.getByText(/grupo de voluntarios trabajando arduamente/i))
        .toBeVisible();
  });
  ...
  test('Nuestra Razón de Ser section heading is visible', async ({page}) => {
    const heading =
        page.getByRole('heading', {name: 'Nuestra Razón de Ser', exact: true});
  ...
  test('Nuestra Misión card heading and text are visible', async ({page}) => {
    ...
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
    await expect(page.getByText(/programas de esterilización y educación/i))
        .toBeVisible();
  });

  test('Nuestra Visión card heading and text are visible', async ({page}) => {
    const heading =
        page.getByRole('heading', {name: 'Nuestra Visión', exact: true});
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
    await expect(page.getByText(/reducción significativa en el maltrato/i))
        .toBeVisible();
  });
  ...
  test('Nuestros Valores heading is visible', async ({page}) => {
    const heading =
        page.getByRole('heading', {name: 'Nuestros Valores', exact: true});
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
  });

  test(
      'all three values are visible (Protección, Compasión, Compromiso)',
      async ({page}) => {
        for (const value of ['Protección', 'Compasión', 'Compromiso'] as
             const) {
  ...
  test('Nuestra Comunidad en Acción heading is visible', async ({page}) => {
    const heading = page.getByRole(
        'heading', {name: 'Nuestra Comunidad en Acción', exact: true});
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
  });
  ...
  test('Nuestro Equipo heading is visible', async ({page}) => {
    const heading =
        page.getByRole('heading', {name: 'Nuestro Equipo', exact: true});
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
  });
  ...
  test('all three team members are visible', async ({page}) => {
    for (const name
             of ['Voluntarios CPAAA', 'Hogares Temporeros', 'Comunidad'] as
         const) {
  ...
  test('Contáctenos heading is visible', async ({page}) => {
    const heading =
        page.getByRole('heading', {name: 'Contáctenos', exact: true});
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
  });
  ...
  test('Facebook link is present', async ({page}) => {
    const link =
        page.getByRole('link', {name: /Visita nuestra página de Facebook/i});
    await link.scrollIntoViewIfNeeded();
    await expect(link).toBeVisible();
  ...
  test('CTA heading is visible', async ({page}) => {
    const heading = page.getByRole(
        'heading',
        {name: 'Tú También Puedes Hacer la Diferencia', exact: true});
    await heading.scrollIntoViewIfNeeded();
    await expect(heading).toBeVisible();
  });
